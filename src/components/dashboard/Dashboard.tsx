...
export function Dashboard({
  ...
  return (
    <div className="max-w-4xl mx-auto text-black">
      ...
      {/* Dynamic components (from agent) */}
      {dynamicComponents.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {dynamicComponents.map((component) => renderComponent(component))}
        </div>
      )}

      {/* Static components (from sample data) - only show if no research data and no dynamic components */}
      {!data && dynamicComponents.length === 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
